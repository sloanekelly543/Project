...
contract Generator is ERC20, Ownable {
    ...
    GNTDividendTracker public dividendTracker;
    
    address  payable public rewardsPool;
    
    // Disabling trading to begin with to avoid situations with other users adding liquidity.
    ...
    constructor() public ERC20("Generator", "GNT") {
    	...
    	dividendTracker = new GNTDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "GNT: The dividend tracker already has that address");

        GNTDividendTracker newDividendTracker = GNTDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "GNT: The new dividend tracker must be owned by the GNT token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "GNT: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "GNT: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "GNT: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "GNT: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "GNT: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "GNT: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "GNT: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if( 
            ...
            if(timeSinceLaunch <= 1 hours && from == uniswapV2Pair && !_isExcludedFromFees[to]){
                if(timeSinceLaunch <= 10 * 1 minutes) { require(amount <= (100000 * (10**18)), 'Amount in first 10 minutes must be less than 100k'); }
                    else if(timeSinceLaunch > 10 * 1 minutes && timeSinceLaunch <= 15 * 1 minutes) { require(amount <= (200000 * (10**18)), 'Amount in first 15 minutes must be less than 200k'); }
                    else if(timeSinceLaunch > 15 * 1 minutes && timeSinceLaunch <= 30 * 1 minutes) { require(amount <= (400000 * (10**18)), 'Amount in first 30 minutes must be less than 400k'); }
                    else if(timeSinceLaunch > 30 * 1 minutes && timeSinceLaunch <= 60 * 1 minutes) { require(amount <= (800000 * (10**18)), 'Amount in first 60 minutes must be less than 800k'); }
        }
